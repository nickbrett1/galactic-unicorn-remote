# Deployment runbook: galactic-unicorn-remote
...
The repository's Buildkite pipeline (`.buildkite/pipeline.yml`) contains a
`docker_publish` step that builds the image for `linux/amd64` and
pushes it to `ghcr.io/nickbrett1/galactic-unicorn-remote` on every push to `main` (tagged with the commit
SHA and `latest`).
...
(`ghcr.io/nickbrett1/galactic-unicorn-remote:buildcache`) and
...
### One-time CI setup (agent `environment` hook)
...
2. In `docker-compose.yml`, change the image to the pinned SHA:
   `image: ghcr.io/nickbrett1/galactic-unicorn-remote:<commit-sha>`
