...
# "galactic-unicorn-remote-dev" (the repo name plus the "-dev" suffix).
...
# than adding a second one - a stale "galactic-unicorn-remote-dev" is overwritten by
...
PROJECT_NAME="galactic-unicorn-remote"
AGENT_NAME="galactic-unicorn-remote-dev"
WORKSPACE="/workspaces/galactic-unicorn-remote"
...
if [ -x "/workspaces/galactic-unicorn-remote/scripts/agent-dev.sh" ]; then
  "/workspaces/galactic-unicorn-remote/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
